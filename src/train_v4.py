...
def evaluate_loss_per_sentence(model, data_loader, criterion, model_type, device, multi_gpu=False):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            sentence_losses = criterion(outputs, targets).cpu().numpy()
            sentence_perplexities = np.exp(sentence_losses)
            all_losses.extend(sentence_losses)
            all_perplexities.extend(sentence_perplexities)
...
def train_model(model, train_loader, val_loader, epochs, learning_rate, model_type, model_name, patience=3, device=None, multi_gpu=False, vocab=None, word_to_index=None, best_params = None, n_gram = None, args = None):
    ...
    model_dir = args.model_dir if args else "./models"  # Use the model_dir argument or default
    save_path = os.path.join(model_dir, f'best_{model_name}_final_model.pth')
    if model_type == 'ffnn':
...
def tune_hyperparameters(text_path, model_type, data_preparation_func, dataset_class, collate_fn=None, n_gram_size=None, n_trials=10, seed = SEED):
    ...
    def objective(trial):
        ...
        trained_model = train_model(model, train_loader, val_loader, epochs=args.epochs, learning_rate=learning_rate, model_type=model_type, model_name=f'tune_{model_type}', device=device, multi_gpu=multi_gpu, vocab=pp_vocab, word_to_index=pp_word_to_index, best_params = {'embedding_dim':embedding_dim, 'hidden_dim':hidden_dim, 'learning_rate':learning_rate, 'dropout_prob':dropout_prob, 'num_rnn_layers':num_rnn_layers} if model_type in ['rnn','lstm'] else {'embedding_dim':embedding_dim, 'hidden_dim':hidden_dim, 'learning_rate':learning_rate, 'dropout_prob':dropout_prob}, n_gram=n_gram_size if model_type == 'ffnn' else None) # Reduced epochs for tuning
...
if __name__ == '__main__':
    ...
    if args.lm_type == 'ffnn':
      ...
      pp_ffnn_N_gram_model = FFNNLM(len(pp_vocab_ffnn_N_gram_), ffnn_N_gram_best_params['embedding_dim'], 3-1, ffnn_N_gram_best_params['hidden_dim'], dropout_prob=ffnn_N_gram_best_params['dropout_prob']).to(device)
